...
class StorageService {
  ...
  Future<void> init({String? testPath}) async {
    ...
    register(TextFileAttachmentAdapter());
    register(ChatSessionAdapter());
    register(SystemPromptAdapter());
    register(ChatPersonaAdapter());
  ...
  Map<String, dynamic> exportBackupData({List<String>? chatIds}) {
    ...
    data['skills'] = getSkills()
        ...
        .toList(growable: false);
    data['memories'] = List<dynamic>.from(
      getSetting(
  ...
  List<Map<String, dynamic>> getLocalNotes() {
  ...
  Future<Map<String, dynamic>> createLocalNote({
    ...
    if (cleanTitle.isEmpty || cleanContent.isEmpty) {
      throw ArgumentError('Note title and content must not be empty.');
    }
    ...
    final notes = [...getLocalNotes(), note];
    await saveSetting(AppConstants.localNotesKey, notes);
    await logActivity('Local Note Created', 'Created note "$cleanTitle"');
    return note;
  }
  ...
  Map<String, dynamic> getExportableSettings() {
    ...
    for (final key in _settingsBox.keys) {
      ...
      if (exportKeys.contains(keyStr) ||
          ...
          keyStr.startsWith(AppConstants.rollingSummaryPrefixKey)) {
  ...
  bool _isAllowedSettingValue(String key, dynamic value) {
    switch (key) {
      ...
      case AppConstants.autoUpdateCheckKey:
      case AppConstants.onlineModelBrowsingKey:
      case AppConstants.strictOfflineModeKey:
      case AppConstants.tavilySearchEnabledKey:
      case AppConstants.githubSkillsEnabledKey:
      case AppConstants.autoMemoryExtractionKey:
      ...
      case AppConstants.messageTemplatesKey:
      case AppConstants.modelManifestRegistryKey:
      case AppConstants.localNotesKey:
  ...
  Future<Map<String, int>> restoreBackupDataAtomically(
    Map<String, dynamic> data, {
    Future<void> Function()? afterStorageWrite,
  }) async {
    final chats = List<dynamic>.from(data['chats'] as List? ?? const [])
        .map(
          (item) => _chatSessionFromJson(
            Map<String, dynamic>.from(item as Map),
          ),
        )
        .toList(growable: false);
    final prompts = List<dynamic>.from(data['prompts'] as List? ?? const [])
        .map(
          (item) => _systemPromptFromJson(
            Map<String, dynamic>.from(item as Map),
          ),
        )
        .toList(growable: false);
    final personas = List<dynamic>.from(data['personas'] as List? ?? const [])
        .map(
          (item) => _personaFromJson(
            Map<String, dynamic>.from(item as Map),
          ),
        )
        .toList(growable: false);
    final skills = List<dynamic>.from(data['skills'] as List? ?? const [])
        .map(
          (item) => _skillFromJson(Map<String, dynamic>.from(item as Map)),
        )
        .toList(growable: false);
    final memories = List<dynamic>.from(
      data['memories'] as List? ?? const [],
    );
    for (final item in memories) {
      final memory = Map<String, dynamic>.from(item as Map);
      if (memory['id'] is! String ||
          memory['fact'] is! String ||
          memory['createdAt'] is! String) {
        throw const FormatException('Backup contains an invalid memory.');
      }
      DateTime.parse(memory['createdAt'] as String);
    }
    ...
    for (final entry in settings.entries) {
      final isModelSetting = entry.key.startsWith(
        AppConstants.modelSettingsPrefixKey,
      );
      final isRollingSummary = entry.key.startsWith(
        AppConstants.rollingSummaryPrefixKey,
      );
      if (isModelSetting && entry.value is Map ||
          isRollingSummary && entry.value is String ||
          allowedKeys.contains(entry.key) &&
              _isAllowedSettingValue(entry.key, entry.value)) {
        safeSettings[entry.key] = entry.value;
      }
    }
    safeSettings[AppConstants.localMemoryRecordsKey] = memories;

    final chatSnapshot = Map<dynamic, ChatSession>.from(_chatBox.toMap());
    final promptSnapshot =
        Map<dynamic, SystemPrompt>.from(_systemPromptBox.toMap());
    final personaSnapshot = Map<dynamic, ChatPersona>.from(_personaBox.toMap());
    final skillSnapshot = Map<dynamic, Skill>.from(_skillsBox.toMap());
    final settingsSnapshot = Map<dynamic, dynamic>.from(_settingsBox.toMap());

    try {
      await _chatBox.putAll({for (final item in chats) item.id: item});
      await _systemPromptBox
          .putAll({for (final item in prompts) item.id: item});
      await _personaBox.putAll({for (final item in personas) item.id: item});
      await _skillsBox.putAll({for (final item in skills) item.id: item});
      await _settingsBox.putAll(safeSettings);
      await afterStorageWrite?.call();
    } catch (error) {
      await _chatBox.clear();
      await _chatBox.putAll(chatSnapshot);
      await _systemPromptBox.clear();
      await _systemPromptBox.putAll(promptSnapshot);
      await _personaBox.clear();
      await _personaBox.putAll(personaSnapshot);
      await _skillsBox.clear();
      await _skillsBox.putAll(skillSnapshot);
      await _settingsBox.clear();
      await _settingsBox.putAll(settingsSnapshot);
      _invalidateCaches();
      throw StateError(
          'Restore failed and existing data was rolled back: $error');
    }

    _invalidateCaches();
    await logActivity(
      'Encrypted Backup Restored',
      'Restored ${chats.length} chats, ${prompts.length} prompts, '
          '${personas.length} personas, ${skills.length} skills, and '
  ...
  ChatPersona _personaFromJson(Map<String, dynamic> json) {
    return ChatPersona(
