...
## 2024-05-29 - [Accessible Dynamic List Items]
**Learning:** In dynamic lists (like image attachments), relying on a static tooltip for removal actions (e.g., "Remove image") creates ambiguity for screen reader users who hear the same label repeatedly.
**Action:** Always include the item's index and total count in both the `Semantics` label of the item (e.g., "Attached image 1 of 3") and the action button (e.g., "Remove image 1 of 3") to provide necessary context and location within the list.
