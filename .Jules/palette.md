...
## 2026-01-21 - [Accessible Custom Paint Charts]
**Learning:** Charts rendered via `CustomPaint` are completely invisible to screen readers, leaving visually impaired users with zero information.
**Action:** Always wrap `CustomPaint` widgets with a `Semantics` widget that provides a dynamic, data-driven summary (e.g., "Total X, Peak Y") in the `label` property to make the data accessible.
