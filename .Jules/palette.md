...
## 2026-01-22 - [Semantic Context for Dynamic Lists]
**Learning:** Visual context (like a list of attached images) is lost on screen readers. "Remove image" repeated 3 times is ambiguous. Providing position context ("Image 1 of 3") in both the item label and the action tooltip allows users to construct a mental model of the list state.
**Action:** When displaying dynamic lists of items (especially attachments), always include "Item X of Y" in the `Semantics` label and associated action tooltips.
