...
## 2025-02-24 - [Consistent Actionable Empty States]
**Learning:** Reusable widgets (like bottom sheets) often hide actions in "compact" modes, but empty states are critical dead-ends regardless of screen real estate. Hiding the primary "Create" action in a modal bottom sheet's empty state forces users to hunt for the header button, increasing friction.
**Action:** Ensure empty state Call-to-Actions (CTAs) remain visible even in compact/modal views, as the empty state provides ample room for them.
