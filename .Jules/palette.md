...
## 2026-01-22 - [Context in Dynamic Lists]
**Learning:** In dynamic lists (like file attachments), screen readers need positional context to be useful. A tooltip like "Remove image" is ambiguous when repeated. "Remove image 1 of 3" allows users to confidently manage content.
**Action:** When rendering lists of interactive items, always include the item's index and total count in the semantic label or tooltip (e.g., "$action $item $index of $total").
