...
## 2026-02-14 - [Active Processing State]
**Learning:** Disabling an action button (turning it grey) while it is processing (e.g., sending a message) breaks the "active" mental model. Keeping the button in its primary color (while showing a loading spinner) signals that the system is "working on it" rather than "stopped" or "invalid".
**Action:** For action buttons that trigger async operations, maintain the primary color during the loading state (with a spinner) instead of reverting to a disabled grey appearance.
